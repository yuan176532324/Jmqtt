...
class MQTTClientLibrary(object):
    ...
    def subscribe(self, topic, qos, timeout=60, limit=1):
    ...
    def subscribe_with_mqttc(self, mqttc, topic, qos, timeout=1, limit=1):
        self._mqttc = mqttc
        self.subscribe(str(topic), int(qos))
        return self._messages

    def publish_with_mqttc(topic, mqttc, message=None, qos=0, retain=False):
        self._mqttc = mqttc
        self.publish(topic, message, int(qos), retain)
